...
PixiJS 版では gsap の `onComplete` を Promise でラップし、全 sprite tween を `Promise.all()` で待つ:

```typescript
async brake(_speed: number): Promise<void> {
  const tweens = this.sprites.map(sprite => {
    /* ... */
    return this.scene.tween(sprite, {
...
- gsap の `eventCallback('onComplete')` が `Promise.all` で待たれているか確認
