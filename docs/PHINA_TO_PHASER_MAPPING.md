...
> **Historical note**: The Phaser 3 implementation has since been migrated to PixiJS 8 + gsap + Howler.js. This document remains as reference for the original phina.js → Phaser port. For the current architecture see [ARCHITECTURE.md](./ARCHITECTURE.md).
    ...
    this.scene.tweens.add({
      ...
      onComplete: () => {
        this.scene.tweens.add({
          targets: this.sprite,
          rotation: {
            from: Phaser.Math.DegToRad(-60),
            to: Phaser.Math.DegToRad(30),
          },
          duration: 80000,
          ease: 'Quad.easeInOut',
          // Continue chain...
